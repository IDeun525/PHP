	...
	<div class="container">
                ...
                <table>
					...
					// 공지사항
                                        ...
                                        $jb_sql = "SELECT * FROM board where notice ='1' order by num desc;";
                                        $jb_result = mysqli_query( $jb_conn, $jb_sql );
                                        while( $jb_row = mysqli_fetch_array( $jb_result ) ) {
						$redate = $jb_row[ 'redate' ];
						if (!is_null ($redate)) {
							echo
                                                	'<tr>
                                                        	<td> ★notice</td>
                                                        	<td><a href="detail.php?num='. $jb_row[ 'num' ].'&page='. $page.'">★'. $jb_row[ 'title' ]. '</a></td>
                                                        	<td>★'. $jb_row[ 'content' ]. '</td>
                                                        	<td>★'. $jb_row[ 'writer' ]. '</td>
								<td>★'. $jb_row[ 'date' ]. ' ('. $jb_row[ 'redate' ] .')</td>
                                                	</tr>';
						} else {
							echo
                                                	'<tr>
					...
					$category = $_GET["category"];
                                        $search= $_GET["search"];
					if (  !is_null( $category ) ) {
                                                $jb_sql = "SELECT * FROM board where $category like '%$search%' order by num desc limit $start, $list_num;";
                                                $jb_count = "SELECT count(*) FROM board where $category like '%$search%';";
                                        } else{
                                                $jb_sql = "SELECT * FROM board order by num desc limit $start, $list_num;";
                                                $jb_count = "select count(*) from board;";
                                        }

                                        $jb_num = mysqli_query( $jb_conn, $jb_count );
                                        $count = mysqli_fetch_array( $jb_num );
                                        $num = $count[ 'count(*)' ];
		...
		<br>
                ...
                <a href="login_ok.php"><button class="btn btn-dark" style="float:right;">돌아가기</button></a>
	</div>
        </body>
</html>
